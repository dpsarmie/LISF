...
!BOP
!
! !ROUTINE: interp_petlvt
! \label{interp_petlvt}
!
!
! !INTERFACE: 
subroutine interp_petlvt(n, findex, nx, ny, finput, lis_gds, nc, nr, varfield)
...
!
! !DESCRIPTION:
!   This subroutine interpolates a given PET LVT field 
!   to the LIS grid. 
!
!  The arguments are: 
!  \begin{description}
...
!  index of the nest
! \item[nx]
!  number of columns (in the east-west dimension) in the PET LVT grid
! \item[ny]
!  number of rows (in the north-south dimension) in the PET LVT grid
! \item[finput]
...
!--------------------------------------------------------------------
  ...
  petlvt_struc(n)%mi = nx * ny

  if( LIS_rc%met_interp(findex) == "bilinear" ) then       ! Bilinear interpolation

     call bilinear_interp( lis_gds,lb,f,lo,lis1d,petlvt_struc(n)%mi,mo, &
          LIS_domain(n)%lat, LIS_domain(n)%lon,&
          petlvt_struc(n)%w111, &
          petlvt_struc(n)%w121,petlvt_struc(n)%w211,petlvt_struc(n)%w221,    &
          petlvt_struc(n)%n111,petlvt_struc(n)%n121,petlvt_struc(n)%n211,    &
          petlvt_struc(n)%n221,LIS_rc%udef, iret )

  else if( LIS_rc%met_interp(findex) == "budget-bilinear" ) then  ! Conservative-budget

    call conserv_interp( lis_gds,lb,f,lo,lis1d,petlvt_struc(n)%mi,mo, &
         LIS_domain(n)%lat, LIS_domain(n)%lon,&
         petlvt_struc(n)%w111, &
         petlvt_struc(n)%w121,petlvt_struc(n)%w211,petlvt_struc(n)%w221,    &
         petlvt_struc(n)%n111,petlvt_struc(n)%n121,petlvt_struc(n)%n211,    &
    ...
    call neighbor_interp( lis_gds,lb,f,lo,lis1d,petlvt_struc(n)%mi,mo, &
         LIS_domain(n)%lat, LIS_domain(n)%lon,&
         petlvt_struc(n)%n111,    &
         LIS_rc%udef, iret )
