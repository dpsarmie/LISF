...
|"'PET LVT'"                   | LVT-generated PET at 0.125 deg
...
==== PET_LVT
anchor:sssec_supp_petlvt[PET_LVT]

'LVT PET forcing directory:` specifies the location of the
LVT-generated PET forcing files.

`LVT PET forcing type:` specifies the choice for PET forcing
 data type.
Acceptable values are:
...
.Example _lis.config_ entry
....
LVT PET forcing directory:       ./PET_LVT
LVT PET forcing type:            current
....
...
`oah-MP.3.6 initial intercepted ice mass:` specifies the Noah-MP-3.6 LSM
...
`HYMAP run in ensemble mode:` specifies whether to run in
ensemble mode.
Acceptable values are:
...
`HYMAP routing model start mode:` specifies if a restart mode is
being used.
Acceptable values are:
