...
!
!   LDT uses a mix of instantaneous forcing values and averaged forcing
!   values from the GDAS forcing data.  LDT processes the GDAS forcing at
!   a 3-hourly period.  When it is time to read new GDAS forcing data,
!   LDT reads 3 hours ahead.  So when LDT' clock is at hour hr, LDT
!   processes data for the period [hr, hr+3].  Here, LDT reads instantaneous
...
! 
!   Again, LDT processes the GDAS forcing at a 3-hourly period.  When the
!   hour of LDT' clock, hr, is one of the analysis hours (00, 06, 12, 18)
!   then LDT reads ahead to extract instantaneous forcing values from
!   the hr.f03 file, and it reads ahead to extract averaged forcing values
!   from the hr.f03 file.
!
!   When the hour of LDT' clock, hr, is not one of the analysis hours
!   (in this case when hr is 03, 09, 15, or 21), then LDT reads instantaneous
!   forcing values from the from the next analysis hour.
!   E.g., when hr is 03, read from 06.f00; when hr is 09, read from 12.f00.
!   LDT computes averaged forcing values by reading the averaged values
...
!   hour thusly:
!
!   Consider LDT' hour to be 15.  LDT needs averaged forcing valid for the
...
!   
!   So to compute the averaged forcing for period [15, 15+3], LDT must
...
!   situation when LDT must subtract $f_3$ values from $f_6$ values to
...
!-----------------------------------------------------------------
  ...
  if ( option == 2 ) then !bookend 2 
     ...
     if ( uhr .eq. 21 ) then
        call LDT_tick(dumbtime,doy,gmt,uyr,umo,uda,uhr,umn,uss,24.0*60*60)
     ...
     if ( uhr0 == 0 ) then
        call LDT_tick(dumbtime,doy,gmt,uyr0,umo0,uda0,uhr0,umn0,uss0,-24.0*60*60)
